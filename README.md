...
| `markets.html` | Where we operate: Reading · Milton Keynes · Thessaloniki. |
| `about.html` | The team, how we operate, what we've done. |
...
- ⚠️ Two rules to keep: **`--terracotta #EE9560` is a fill only** (use `--terracotta-deep #B85C2B`
  for any warm text); and **green (`--forest`) is used only on titles, the wordmark and the footer** —
  don't recolour body text or buttons green.
...
- **File:** `contact.html`, the `.cities` list, and `about.html` team cards. Replace the
...
- To use a hosted form handler instead, set `FORM_ENDPOINT` in the `<script>` at the bottom of
  `contact.html` to its URL — and add a consent/DPA note to the privacy policy first.
